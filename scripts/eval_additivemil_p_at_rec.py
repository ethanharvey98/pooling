...
DATASETS = {
    "Head CT": {"l1_dir": f"{EH}/RSNA_ICH_full_dataset_embedding_level=True",
                "data_dir": f"{DATA}/encoded_RSNA_ICH_full_dataset/ViT_B_16"},
    "Chest CT": {"l1_dir": f"{EH}/RSNA_PE_embedding_level=True",
                 "data_dir": f"{DATA}/encoded_RSNA_PE/ViT_B_16"},
    "Abdomen CT": {"l1_dir": f"{EH}/RSNA_AT_embedding_level=True",
...
def _select(dir_, seed):
    ...
    for alpha in ALPHAS:
        for lr in LRS:
            name = f"alpha={alpha}_criterion=L1_lr={lr}_pooling=AdditiveMIL_seed={seed}"
            csv = f"{dir_}/{name}.csv"
...
def main():
    ...
    pd.DataFrame(out_rows).to_csv("/cluster/home/zmou01/pooling/experiments/_additivemil_p_at_rec.csv", index=False)
    print("wrote _additivemil_p_at_rec.csv")
