"""Precision@10%Recall for the ABMIL + Normal Guidance (ours) row only, across
...
Run from the repo root in the neuroimg_gpu env:
    python scripts/eval_ng_abmil_p_at_rec10.py
"""
import os
import sys
...
def semi_dir(seed):
    return f"{EH}/varying_n_beta=1.0_embedding_level=True/{SEMI_SUFFIX}/{SEMI_SUBDIR[seed]}"


DATASET_CFG = {
    "Semi-Synthetic": {"kind": "semi"},
    "Chest CT (RSNA PE)": {"kind": "std", "ng_dir": f"{EH}/RSNA_PE_beta=1.0_embedding_level=True",
                             "data_dir": f"{DATA}/encoded_RSNA_PE/ViT_B_16"},
    "Abdomen CT (RSNA AT)": {"kind": "std", "ng_dir": f"{EH}/RSNA_AT_beta=1.0_embedding_level=True",
...
def _select(dir_, seed):
    ...
    for alpha in ALPHAS:
        for lr in LRS:
            name = f"alpha={alpha}_criterion=GuidedL1_lr={lr}_pooling=ABMIL_seed={seed}"
            csv = f"{dir_}/{name}.csv"
...
def main():
    ...
    out_csv = "/cluster/home/zmou01/pooling/experiments/ng_abmil_p_at_rec10_3datasets.csv"
