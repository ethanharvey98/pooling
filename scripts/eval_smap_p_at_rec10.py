"""Precision@10%Recall for the Smooth Operator (SmAP) row only, across
...
Run from the repo root in the neuroimg_gpu env:
    python scripts/eval_smap_p_at_rec10.py
"""
import os
import sys
...
def _select(dir_, seed):
    ...
    for alpha in ALPHAS:
        for lr in LRS:
            name = f"alpha={alpha}_criterion=L1_lr={lr}_pooling=SmAP_seed={seed}"
            csv = f"{dir_}/{name}.csv"
...
def main():
    ...
    for tag, cfg in DATASET_CFG.items():
        ...
        for seed in SEEDS:
            ...
            m = models.PoolClf(768, 1, pooling="SmAP")
    ...
    out_csv = "/cluster/home/zmou01/pooling/experiments/smap_p_at_rec10_3datasets.csv"
