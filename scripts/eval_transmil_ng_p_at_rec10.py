"""Precision@10%Recall for the TransMIL + NG row only, across
...
Run from the repo root in the neuroimg_gpu env:
    python scripts/eval_transmil_ng_p_at_rec10.py
"""
import os
import sys
...
def _select(dir_, seed):
    ...
    for alpha in ALPHAS:
        for lr in LRS:
            name = f"alpha={alpha}_criterion=GuidedL1_lr={lr}_pooling=TransMIL_seed={seed}"
            csv = f"{dir_}/{name}.csv"
...
def main():
    ...
    out_csv = "/cluster/home/zmou01/pooling/experiments/transmil_ng_p_at_rec10_3datasets.csv"
