...
def main():
    ...
    model_path, val_auroc = find_best_model(args.experiments_dir, args.pooling, args.seed)
    if not model_path:
        raise ValueError(f"No valid model found for pooling={args.pooling}, seed={args.seed}")
    ...
    attention_weights = get_attention(model, X, lengths, args.embedding_level)

    # Recreate the test split to get scan IDs and slice-level labels
    test_scan_ids, test_slice_labels = get_test_slice_labels(args.labels_csv, args.numpy_dir, args.seed)
