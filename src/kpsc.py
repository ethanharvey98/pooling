...
if __name__=='__main__':
    ...
    if args.embedding_level:
        model = models.PoolClf(in_features=train_data['X'].shape[1], out_features=1, pooling=args.pooling, **pool_kwargs)
    else:
        model = models.ClfPool(in_features=train_data['X'].shape[1], out_features=1, pooling=args.pooling, **pool_kwargs)
