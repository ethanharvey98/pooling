...
class PPEG(torch.nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        out = torch.cat([torch.cat((cls_token[i][None,:], x_i)) for i, x_i in enumerate(torch.split(x, lengths))])
        return out
...
class Sm(torch.nn.Module):
    def __init__(self, alpha=0.5, learnable_alpha=True, neighbors=1, self_loop=False):
        super().__init__()
...
class ApproxSm(Sm):
    def __init__(self, alpha=0.5, learnable_alpha=True, neighbors=1, num_steps=10, self_loop=False):
        super().__init__(alpha=alpha, learnable_alpha=learnable_alpha, neighbors=neighbors, self_loop=self_loop)
...
class ExactSm(Sm):
    ...
    def _adajency_matrix(length, device=None, dtype=None):
...
class smAP(torch.nn.Module):
    def __init__(self, in_features, hidden_dim=128, alpha=0.5, num_steps=10, variant='early'):
        super().__init__()
    ...
    def forward(self, x, lengths):
        ...
        if self.variant == 'late':
            ...
            mid = torch.cat([
                self.sm_layer(mid_i)
                for mid_i in torch.split(mid, lengths)
            ])
