...
def train_one_epoch(model, criterion, optimizer, dataloader, lr_scheduler=None):
    ...
    metrics = {'auroc': 0.0, 'auprc': 0.0, 'bal_acc': 0.0, 'labels': [], 'logits': [], 'loss': 0.0, 'nll': 0.0, 'kl': 0.0}
    ...
    for images, lengths, labels in dataloader:
        ...
        losses = criterion(logits, labels, attn_weights=attn_weights, lengths=lengths, params=params, N=len(dataloader.dataset), model=model)
        ...
        if 'kl' in losses:
            metrics['kl'] += (batch_size / dataset_size) * losses['kl'].item()
...
def evaluate_with_uncertainty(model, dataloader, n_samples=10):
    ...
    with torch.no_grad():
        for images, lengths, labels in dataloader:
            if device.type == 'cuda':
                images, labels = images.to(device), labels.to(device)
            result = model.predict_with_uncertainty(images, lengths, n_samples=n_samples)
            if device.type == 'cuda':
                result = {k: v.cpu() for k, v in result.items()}
            result['labels'] = labels.cpu() if device.type == 'cuda' else labels
